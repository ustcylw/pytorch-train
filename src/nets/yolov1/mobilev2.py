...
class MobileNetV2(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        x = self.features(x)
        # print(f'[_forward_impl][1]  {x.shape=}')
        # # Cannot use "squeeze" as batch-size can be 1
        # x = nn.functional.adaptive_avg_pool2d(x, (1, 1))
        # print(f'[_forward_impl][2]  {x.shape=}')
        # x = torch.flatten(x, 1)
        # print(f'[_forward_impl][3]  {x.shape=}')
        x = self.classifier(x)
        return x
...
def get_mobilenet_v2(pretrained=False):
    ...
    net.classifier = torch.nn.Sequential(
        torch.nn.Conv2d(net.last_channel, 30, kernel_size=1, stride=1, padding=0, bias=False),
