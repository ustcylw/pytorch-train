...
class yoloDataset(data.Dataset):
    ...
    def __init__(self,root,list_file,train,transform):
        ...
        if isinstance(list_file, list):
            ...
            print(list_file)
            tmp_file = '/data/ylw/code/pl_yolo/data/yolov1/tmp/listfile.txt'
    ...
    def __getitem__(self,idx):
        ...
        img_file = os.path.join(self.root,fname)
        img = cv2.imread(img_file)
        if not os.path.exists(img_file) or img is None:
            idx = 1
            fname = self.fnames[idx]
            img_file = os.path.join(self.root,fname)
            img = cv2.imread(img_file)
    ...
    def yolo_dataset_collate(self, batch):
        ...
        # print(f'[99999999999999]  {type(imgs)}  {type(imgs[0])}  {type(gt)}')
        # images  = torch.from_numpy(np.array(imgs)).type(torch.FloatTensor)
        # gts  = [torch.from_numpy(gt).type(torch.FloatTensor) for gt in gts]
        images = torch.cat(imgs).type(torch.FloatTensor)
        gts  = torch.cat(gts).type(torch.FloatTensor)
...
def main():
    ...
    for i in range(10):
        ...
        print(f'[]  {img.shape}, {img.device}, {target.shape}  {target.device}')
        img = img.to(0)
        target = target.to(0)
        print(f'[1]  {img.shape}, {img.device}, {target.shape}  {target.device}')
        img = img.to(1)
        target = target.to(1)
        print(f'[2]  {img.shape}, {img.device}, {target.shape}  {target.device}')
